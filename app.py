...
def submitting_unique_code():
    ...
    if request.method == 'POST':
        ...
        print("This is the business sector you used during login as the manager: ", business_sector)
        ...
        if unique_code:
            ...
            if code_sector is None or code_sector[0] != business_sector:
                flash("The clearance for this unique code is either not found or allowed for your login credentials.")
                connection.close()
            ...
            if not rows and not maturity_data:
                flash("No records found for the provided unique code.")
                return render_template('manager.html', error_message=error_message,
                                       business_data=business_functions_data, maturity_data=maturity_data, user_name=user_name)
    ...
    return render_template('managerMaturityResults.html', error_message=error_message,
...
def ValueChainRecommendation():
    user_name = session.get('name', 'User')  # Retrieve the user's name from session
    recommendations = []  # To store the fetched value chain data
    plot_image = None  # To store the plot image if needed
    ...
    if request.method == 'POST':
        ...
        # Fetch BusinessProcess, Activities, Skills, Technology from the ValueChain table
        cursor.execute('''
        ...
        recommendations = cursor.fetchall()  # Fetch the data from the query
        
        if recommendations:
            for record in recommendations:
                # Split the skills and technology strings into lists
                skills_list = record[2].split(", ")  # Splitting skills by comma
                technology_list = record[3].split(", ")  # Splitting technology by comma
                print("Skills:", skills_list)
                print("Technology:", technology_list)
        
        connection.commit()  # Ensure the commit is called as a function
        # Close the database connection
        connection.close()



    # Render the template and pass the recommendations and plot_image
    return render_template('managerRecommendations.html', recommendations=recommendations, plot_image=plot_image, user_name=user_name)
