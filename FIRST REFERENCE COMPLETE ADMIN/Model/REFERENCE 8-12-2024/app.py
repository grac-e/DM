...
def select_business_function():
    ...
    if request.method == 'POST':
        ...
        if selected_business_function:
            ...
            # Loop through the selected data, generate questions, and store them in session
            for data in business_function_selected_data:
                ...
                # Generate AS-IS and TO-BE questions
                as_is_question = f"For the lists of activities of the {measuring_elt} process, how would you describe your current procedure?"
                to_be_question = f"For the lists of activities of the {measuring_elt} process, where do you want your procedure to be in the future?"
    ...
    return render_template('BusinessAnalyst.html', user=user, user_photo_base64=user_photo_base64,
                           ...
                           business_sector_rating=business_sector_rating)
...
def CombinedTiersForUser():
    ...
    if request.method == 'POST':
        ...
        selected_business_function = session.get('selected_business_function', None)
        # Fetch the as_is and to_be questions from the session
        next_questions_section_dict = session.get('as_is_to_be_questions', {})

        # Check if all required fields are present
        if not answers_user_as_is or not answers_user_to_be or not measuring_element_name_user or not sub_category_name_user:
        ...
        try:
            ...
            for i in range(len(measuring_element_name_user)):
                measuring_elt = measuring_element_name_user[i]
                sub_category = sub_category_name_user[i]

                # Retrieve the corresponding AS-IS and TO-BE questions from the session dictionary
                as_is_questions_user = next_questions_section_dict.get(measuring_elt, {}).get('as_is_question', '')
                to_be_questions_user = next_questions_section_dict.get(measuring_elt, {}).get('to_be_question', '')

                rating_user_val = float(request.form.getlist('Rating_User[]')[i])
                ...
                # Insert the data into the UserSubmissionRecord table
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (UserSubmittedUniqueCode, selected_business_function, measuring_elt, rating_user_val,
                      sub_category, as_is_questions_user, user_answer_rating_as_is,
                      to_be_questions_user, user_answer_rating_to_be, max_rating_user,
                      expected_cum_sum, user_cum_sum_as_is, user_cum_sum_to_be))

            connection.commit()

            # Call the feedback function to process feedback after submission
            feedback_function()

            # Redirect to the Business Analyst dashboard after successful submission
            return redirect('/BusinessAnalysts')

        except IndexError as e:
            error_display_asistobe = "An error occurred during form submission: " + str(e)
            return render_template('BusinessAnalyst.html', error_display_asistobe=error_display_asistobe)

        finally:
            connection.close()

    # Render the Business Function page if not a POST request
    return render_template('BusinessAnalyst.html', error_display_asistobe=error_display_asistobe)
...
def value_chain_process_csv(csv_file):
    ...
    for row in csv_data:
        if len(row) != 7:
            raise ValueError("CSV file must have exactly 7 columns")

        id, business_sector, business_function, business_process, business_activities, business_skills, business_technologies = row

        cursor.execute('''
            INSERT INTO ValueChain (id, BusinessSector, BusinessFunction, BusinessProcess, Activities, Skills, Technology)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (id, business_sector, business_function, business_process, business_activities, business_skills,
...
def ClassifyingDigitalMaturity(business_sector, business_function_selected):
    ...
    if business_sector and business_function_selected:
        try:
            ...
            Business_Sector_encoded = label_encoder_sector.transform([business_sector])[
                                                                     0]
            Business_Function_encoded = label_encoder_function.transform(
                [business_function_selected])[0]
            ...
            features = np.array(
                [[Business_Sector_encoded, Business_Function_encoded]])
            prediction_encoded = Extra_Tree_model.predict(features)[0]

            # Inverse transform the prediction to get the categorical label
            prediction_label = label_encoder_prediction.inverse_transform(
                [prediction_encoded])[0] + " " + "Maturity"
...
def submitting_unique_code():
    ...
    return render_template('manager.html', error_message=error_message,
                           ...
                           pie_chart_to_be=pie_chart_to_be, feedback_data=feedback_data)
...
def ValueChainRecommendation():
    ...
    if request.method == 'POST':
        ...
        print(f"Business Sector: '{business_sector_name}'")
        ...
        # Execute the SQL select query with proper syntax
        cursor.execute('''
            SELECT Skills, Technology
            FROM ValueChain
        ...
        # Debugging: Print out the recommendations and the SQL statement executed
        print("Executed SQL:", cursor)
        print("Recommendations:", recommendations)

        connection.close()

    return render_template('managerRecommendations.html', recommendations=recommendations, business_data=business_functions_data)
