...
def feedback_function():
    ...
    for avg_record in average_percentages:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (business_sector, UniqueCodeUser, 'Digital Maturity', BusinessFunction, avg_percentage_as_is, avg_percentage_to_be))
        ...
        if unique_code:
            ...
            if not rows:
                ...
                return render_template('manager.html', error_message=error_message,
                                       business_data=business_functions_data)
...
@app.route('/classifying_digital_maturity', methods=['GET', 'POST'])
def ClassifyingDigitalMaturity():
    business_functions_data = {}
    # Retrieve session values
    business_sector_session = session.get('business_sector', None)
    prediction_label = None
    if request.method == 'POST':
        # Extract the selected business function from the form
        business_function_selected = request.form.get('business_function_user', None)

        if business_sector_session and business_function_selected:
            # Encode using LabelEncoder
            Business_Sector_encoded = label_encoder_sector.transform([business_sector_session])[0]
    ...
    # Pass prediction_label to the template
    return render_template('manager.html', prediction_label=prediction_label, business_data=business_functions_data)
